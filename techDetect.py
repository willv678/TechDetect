...
def faceFound():
    ...
    if not faceName in loggedIn:
        ...
        print(faceName + "'s student id is: " + mainStudentID)
        #print(todayDate + " is toda's date idk i hope this work")
        sql = "INSERT INTO attendance_"+todayDate+" (studentID, studentName, studentCheckInTime) VALUES (%s, %s, %s)"
        getTime()
        todayDate = todayDate.replace("-","_")
        #print("Current time is " + currentTime)
        val = (mainStudentID, faceName, currentTime)
        #val = (19999, "Will", "2:00")
...
def exit():
    ...
    for x in loggedIn:
        ...
        #print("Logging out ", x, " ...")
        print("Clocking out user", x, "at", datetime.now())
        #print(todayDate)
        quitInfo = (x, " - ",datetime.now())
        ...
        mycursor.execute("UPDATE `techdetect`.`attendance_"+todayDate+"` SET `studentCheckOutTime` = '"+ currentTime+"' WHERE (`studentName` = '"+ x+"')")
        mydb.commit()
        print("Successfully checked out " + x + "!")
...
webcam.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # set new dimensionns to cam object (not cap)
...
with mp_hands.Hands(min_detection_confidence=.8, min_tracking_confidence=.8, max_num_hands=2) as hands:
    while True:
        ...
        #cv2.namedWindow('Tech Detect', cv2.WINDOW_NORMAL)
        #cv2.setWindowProperty('Tech Detect', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
